pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    app = docker.build("aplata1970/react-app")
                    app.inside {
        ...
        stage('Push Docker Image') {
         ...
         stage('DeployToStaging') {
            ...
            steps {
                    script {
                        sh "docker pull aplata1970/react-app:${env.BUILD_NUMBER}"
                        try {
                        ...
                        sh "docker run --restart always --name react-app -p 1233:80 -d aplata1970/react-app:${env.BUILD_NUMBER}"
                    }
            }
        }
                ...
                input 'Does the staging environment look OK? Did You get 200 response?'
                 milestone(1)
                    script {
                        ...
                        sh "docker run --restart always --name react-app -p 1233:80 -d aplata70/react-app:${env.BUILD_NUMBER}"
                    }
            }
        }
    }
